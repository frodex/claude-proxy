# Add terminal & unified launch profiles — Research (stepped v01)

**Date:** 2026-04-03  
**Status:** In Progress — research for PRD amendment & implementation plan  
**Preceding:** `add-terminal.md` (baseline; **frozen** — do not edit)  
**Changes from baseline:** Stepped copy per project convention; added §0 stepped workflow for this document family  
...
- **`scheduleClaudeIdBackfill`** — Polls for Claude session ID after attach; **only meaningful for Claude** (should be gated by profile).THIS WOULD ALSO BE TRUE FOR CURSOR AS MY HOPE IS THAT CURSOR AND CLAUDE SESSIONS CAN BEOMCE INTERCHANGABLE AT SOME POINT.  SO THIS CREATES A NEW CHALLANGE WERE AGENTIC TASKS THAT LAUNCH CURSOR OR CLAUDE SHOULD BE ABLE TO USE THE SAME CONFIG FILES AND THE FLAVOR CURSOR/CLAUDE/ETC SHOULD COME FROM THE FILE AS TO GUIDE THE FLOW OF WHICH APPLICATION IS LAUCHED WHEN CALLING UP A PREVIOUS SESSIONS OR FORKING, ETC.
...
- **`src/session-form.yaml`** + **`buildSessionFormSteps`** — Modes: `create`, `edit`, `restart`, `fork`. Field visibility/lock/prefill per mode. WE LIKE;Y NEED TO GRAY OR REMOVE OPTIONS SUCH AS FORK FROM "NON-CLAUDE" TYPE PROCESSESS, WE DON'T SUPPORT FORKING ON. CURRENTLY ONLY CLAUDE SUPPORTS FORKING, PERHAPS CURSOR WILL ALSO BUT NOT AT THIS TIME. I LIKE TO KEEP THESE SORTS OF OPTIONS IN A CONFIG FILE FOR THE OPTION. SO IF WE ADDED A NEW TERMINAL LAUNCHING OPTION FOR SAY TOP THE SETUP OPTIONS FOR THAT SHOULD PULL IN FROM A CONFIG**`finalizeSessionFromResults`** passes through `dangerousSkipPermissions` and resume-related args only in resume/fork paths — all assume Claude for **create**.
...
- Replace generic “New session” with explicit options, e.g. **New Claude session**, **New Cursor session**, **New terminal** (or a two-step: pick type → same wizard). Internally each sets a **`launchProfileId`** and enters **one** flow. I LIKE THE TWO STEP OPTION, BUT ALSO THE ABILITY IN THE API TO JUMP PAST THIS, IF YOU HAVE THE SESSION SETUP YOU CAN JUMP TO THAT FLOW USING THE API. DOES THIS MAKE SENCE?

### 3.2 Single internal flow

- **One** `startSessionCreationFlow(client, profileId)` (name TBD).
- **One** finalize path: `finalizeSessionFromResults(client, results, { profileId, … })` that resolves **`command` + `args`** from the **profile** + form results — no per-tool `finalizeClaude` / `finalizeShell` copies. I THINK THE API SHOULD ALLOW INJECTION OF ANY PROMPTS IT NEEDS TO A TERMINAL SESSIONS. SO VIA API YOU CAN LAUNCH ANYTHING YOU NEED. THIS COULD BE DANGEROUS - WE MIGHT WANT TO CONSIDER FILTERS?
...
*Stepped v01 — next assistant edit: create v02 via `cp` after processing any `add-terminal.v01-NOTES-01.md`.*
