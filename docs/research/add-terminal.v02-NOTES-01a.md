...
- **One** `startSessionCreationFlow(client, profileId)` (name TBD) for SSH; optional `profileId` + full field map for API. NEW TERMINAL , NEW CLAUDE SESSION, NEW CURSOR SESSION ( RUNS CURSOR-AGENT IN THE TARGET DIRECTORY, WILL NEED TO MODIFY TOOLS TO GET SESSION ID THE SAME WAY AS IT DOES FOR CLAUDE (SUB-PROJECT))
